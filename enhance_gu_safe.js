...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('  gu: {')) {
    inGuBlock = true;
    continue;
  }
  
  if (inGuBlock && lines[i].trim() === '},' || lines[i].trim() === '}') {
    // If we hit the end of gu block (which actually is `  },` at line 2197)
    // Wait, let's just use `  },` specifically.
  ...
  if (inGuBlock) {
    let line = lines[i];
    // Simple parse: check if line starts with `'Key':`
    let match = line.match(/^\s*'([^']+)'\s*:/);
    if (!match) {
        // Try double quotes or other formats just in case, though they are single quoted
        match = line.match(/^\s*"([^"]+)"\s*:/);
    }
    
    if (match) {
      let key = match[1];
      // Unescape key for lookup
      let unescapedKey = key.replace(/\\'/g, "'");
      
      if (improvedGu[unescapedKey]) {
        let safeVal = improvedGu[unescapedKey].replace(/'/g, "\\'");
        // Replace the value
        // The regex replaces everything after the colon
        lines[i] = line.replace(/(:\s*').*?(',?)$/, `$1${safeVal}$2`);
        replacementsCount++;
        delete improvedGu[unescapedKey];
      }
    }
  }
}

missingKeys = Object.keys(improvedGu);
...
if (missingKeys.length > 0) {
    console.log(missingKeys.slice(0, 10)); // print first 10
}

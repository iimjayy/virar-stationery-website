...
let lines = fs.readFileSync('assets/js/features/language-toggle.js', 'utf8').split('\n');

// Find the end of `gu: {` (line containing `  },` around 2197)
let guEndIdx = -1;
...
// Find the end of DICT (line containing `};`)
let dictEndIdx = -1;
for (let i = guEndIdx + 1; i < lines.length; i++) {
  if (lines[i] === '};') {
    dictEndIdx = i;
    break;
  }
...
if (guEndIdx === -1 || dictEndIdx === -1) {
  console.log("Could not find guEndIdx or dictEndIdx", guEndIdx, dictEndIdx);
  process.exit(1);
}

// Extract strings from root
let rootStrings = [];
for (let i = guEndIdx + 1; i < dictEndIdx; i++) {
  let line = lines[i];
  ...
  if (match) {
    rootStrings.push({ key: match[1], val: match[2] });
  }
}

// Remove them from the file
lines.splice(guEndIdx + 1, dictEndIdx - guEndIdx - 1);

// We need to inject them into mr, hi, gu.
// First, prepare the blocks to insert.
let mrLines = [];
let hiLines = [];
let guLines = [];

// Use improved Gujarati if available
const improvedGu = {
  "Printing": "પ્રિન્ટિંગ",
...
for (let item of rootStrings) {
  let k = item.key;
  let mrHiVal = k.replace(/'/g, "\\'"); // Fallback to English for mr/hi so it doesn't break
  let guVal = improvedGu[k.replace(/\\'/g, "'")] ? improvedGu[k.replace(/\\'/g, "'")].replace(/'/g, "\\'") : item.val.replace(/'/g, "\\'");
  
  mrLines.push(`    '${k.replace(/'/g, "\\'")}': '${mrHiVal}',`);
  hiLines.push(`    '${k.replace(/'/g, "\\'")}': '${mrHiVal}',`);
  guLines.push(`    '${k.replace(/'/g, "\\'")}': '${guVal}',`);
}

// Find insertion points
let mrEnd = lines.findIndex(l => l.startsWith('  },') && lines.indexOf(l) > lines.indexOf('  mr: {'));
lines.splice(mrEnd, 0, ...mrLines);

let hiEnd = lines.findIndex(l => l.startsWith('  },') && lines.indexOf(l) > lines.indexOf('  hi: {'));
lines.splice(hiEnd, 0, ...hiLines);

let guEndFinal = lines.findIndex(l => l.startsWith('  },') && lines.indexOf(l) > lines.indexOf('  gu: {'));
lines.splice(guEndFinal, 0, ...guLines);

fs.writeFileSync('assets/js/features/language-toggle.js', lines.join('\n'));
console.log('Fixed root strings and injected ' + rootStrings.length + ' translations safely.');
